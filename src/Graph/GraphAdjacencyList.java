...
public class GraphAdjacencyList {
    ...
    private Vertex1[] vertList;

    public GraphAdjacencyList(int maxVerts) {
        this.maxVerts = maxVerts;
        numVerts = 0;
        adjList = new ArrayList<List<Integer>>();
        vertList = new Vertex1[maxVerts];
    ...
    public void addVertex(char label) {
        if (numVerts < maxVerts) {
            vertList[numVerts++] = new Vertex1(label);
        }
    }
    ...
    public void displayAllEdges() {
        for (int i = 0; i < adjList.size(); i++) {
            ...
            for (int j = 0; j < adjVertex.size(); j++) {
                Integer str = adjVertex.get(j); // inner list stores index of adjacent Vertex
